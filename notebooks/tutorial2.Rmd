...
run_jags <- function(df, model_file) {
  ...
  X <- append_time(X)  # append the time column to X
...
for (subject in sub_list) {
  for (feature in features_list) {
    ...
    # run jags
    data.r2jags <- run_jags(dat_df, file.path("..", "likelihood_models", "fact_anovaModel_default.txt"))
...
output_folder <- file.path("..", "data", "ANCOVA_with_time")
dir.create(output_folder, showWarnings = FALSE)
write.csv(
  all_estimate_df, 
  file = file.path(output_folder, "all_estimates.csv"), 
  row.names=FALSE
)
...
```{r} 
# calculate mean for dt_control (the "reference" group, beta[1])
D <- dat_df[dat_df$fatigue == "control" & dat_df$condition == "dt",]
mean(D$y)
```
