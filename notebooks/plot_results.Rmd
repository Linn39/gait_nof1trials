...
# add sd over all subjects
# conditions_df_sd_all <- sqrt(sum(conditions_df_sd["sd"]^2))
conditions_df_sd_all <- aggregate(sd~condition+var, conditions_df_sd, function(x) sqrt(sum(x^2)))
...
ggplot(all_strides_df, aes(x = sub, y=stride_times, fill = conditions)) +
  geom_boxplot() +
  scale_fill_brewer(palette="Spectral") +
  theme(axis.text.x = element_text(angle = 45))
