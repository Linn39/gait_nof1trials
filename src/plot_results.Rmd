...
```{r, fig.width = 7, fig.height = 5}
fig_posterior_prob <- ggplot(filter(all_estimate_df, parameter == 'p1'), aes(x=as.factor(sub), y=mean, fill=var)) +
...
# plot mean changes from the observed data
fig_posterior_diff_fatigue <- ggplot(diff_fatigue_plot_df, aes(x=as.factor(sub), y=value, fill = var)) +
  geom_bar(position=position_dodge(), stat="identity", colour='black') + 
