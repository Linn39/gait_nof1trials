...
# # read posterior estimates from file
model <- "AR1_model_diff_prob"
downsample_n <- 5
read_path <- file.path(
  "data",
  "processed",
  paste0("all_left_foot_downsample_", downsample_n)
)

# load posterior estimates

if (file.exists(file.path(  # if summary of the posterior estimates exists, load summary
  read_path,
  paste0("all_estimates_", model, ".csv")
  ))) {
    all_estimates <- read.csv(
      file.path(
        read_path,
        paste0("all_estimates_", model, ".csv"),
      row.names = 1
...
fig_posterior_prob <- ggplot(filter(all_estimate_df, (parameter == 'p1'))) + 
  ...
  labs(color='Gait Parameter', x = 'Participant', y = 'Probability of Meaningful Change')
