...
downsample_n <- 5
file_name <- "all_estimates_AR1.csv"
read_path <- file.path("..", "data", "processed", paste0("cauchy_conjugate_left_foot_downsample_", downsample_n))
file_path <- file.path(read_path, file_name)
all_estimate_df <- readr::read_csv(file_path, col_names = T, show_col_types = FALSE)
...
write.csv(
plot_df, 
file = file.path(read_path, paste0("all_estimates_gait_parameters_", model_names[[model_n]], ".csv")), 
row.names=FALSE
)
...
p <- ggplot(plot_df, aes(x = Participant, fill = Condition)) +
...
p <- p + facet_wrap(vars(var), nrow = 2, strip.position = "left", scales = "free_y") +
...
p <- ggplot(all_observations_df, aes(x = Participant, y=value, fill = Condition)) +
...
p <- p + facet_wrap(vars(variable), nrow = 2, strip.position = "left", scales = "free_y") +
  theme(strip.background=element_rect( fill="white"), 
        strip.placement = "outside",
        panel.spacing.y = unit(1, "lines")
        )
...
p <- ggplot(diff_df_plot, aes(Participant, Conditions, fill = Difference)) + 
...
p <- p + facet_grid(rows = vars(variable), switch = "y") +
  theme(strip.background=element_rect( fill="white"),
        strip.placement = "outside",
        panel.spacing.y = unit(1, "lines")
        )
