...
for (var in c("stride_lengths", "stride_times")) {
  file_path <- file.path(read_path, paste0("all_estimates_", var, "_", model_names[[model_n]], ".csv"))
  estimate_df <- readr::read_csv(file_path, col_names = T, show_col_types = FALSE)
  estimate_df$var <- var
  all_estimate_df <- bind_rows(all_estimate_df, estimate_df)
}
